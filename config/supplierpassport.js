...
module.exports = function(passport) {
  ...
  passport.deserializeUser(function(id, done) {
    myconnection.query("SELECT * FROM suppliers WHERE id = ? ", [id], function(
      err,
      rows
    ) {
      done(err, rows[0]);
  ...
  passport.use(
    ...
    new LocalStrategy(
      ...
      function(req, username, password, done) {
        myconnection.query("SELECT * FROM suppliers WHERE username = ? ", [username], function(err, rows) {
